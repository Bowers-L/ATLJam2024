...
{
    [SerializeField] List<BulletPattern> patternDeck;
    ...
    public class BulletPatternInstance {
        ...
        public BulletPatternInstance(BulletPattern pattern, float destroyAfter = Mathf.Infinity)
        {
    ...
    private void UpdateBullets(BulletPatternInstance instance)
    {
        ...
        if (instance.fireTimer <= 0.0f)
        {
            ...
            {
                float angle = angleStep * (i - instance.pattern.numBullets / 2) + instance.pattern.startAngularOffset;
                ...
                Vector2 spawnPos = (Vector2)transform.position + velDir * instance.pattern.startRadialOffset;
